...
export default class DocsMigrateCommand extends BaseCommand {
    id = 'docs migrate';
    route = 'guides';
    static hidden = true;
    static summary = `Migrates a directory of pages to the ReadMe Guides format.\n\n${alphaNotice}`;
    static description = "The path can either be a directory or a single Markdown file. The command will transform the Markdown using plugins and validate the frontmatter to conform to ReadMe's standards.";
    static args = {
    ...
    async run() {
        ...
        const { path: rawPathInput } = this.args;
        ...
        const fileScanHookResults = await this.config.runHook('pre_markdown_file_scan', zipResults);
        ...
        const validationHookResults = await this.config.runHook('pre_markdown_validation', { pages: unsortedFiles });
        ...
        if (transformedByHooks) {
            const fileUpdateSpinner = ora({ ...oraOptions() }).start(`📝 Writing the updated files to the following directory: ${chalk.underline(outputDir)}...`);
