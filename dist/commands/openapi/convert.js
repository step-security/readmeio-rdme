...
export default class OpenAPIConvertCommand extends BaseCommand {
    id = 'openapi convert';
    static summary = 'Converts an API definition to OpenAPI and bundles any external references.';
    static description = 'Converts Swagger files and Postman collections to OpenAPI and bundles any external references. **Note**: All of our other OpenAPI commands already do this conversion automatically, but this command is available in case you need this functionality exclusively.';
    static args = {
        spec: specArg,
    };
    static flags = {
    ...
    async run() {
        ...
        if (specType === 'OpenAPI') {
            this.warn('The input file is already OpenAPI, so no conversion is necessary. Any external references will be bundled.');
        }
        prompts.override({
            outputPath: out,
        });
        ...
        return Promise.resolve(chalk.green(`Your API definition has been converted and bundled and saved to ${promptResults.outputPath}!`));
    }
}
