...
export default class OpenAPIResolveCommand extends BaseCommand {
     ...
     *
     */
    replaceRefWithObjectProxySchemes(
    ...
    circularRefs) {
        ...
        function replaceRef(schemaName, propertyName, refSchemaName) {
        ...
        function createRefSchema(originalSchemaName, refSchemaName) {
    ...
    schemas) {
        ...
        while (remainingCircularRefs.length > 0 && iterationCount < maxIterations) {
            this.debug(`Iteration ${iterationCount + 1}: Resolving ${remainingCircularRefs.length} remaining circular references.`);
        ...
        if (remainingCircularRefs.length > 0) {
            ...
            while (remainingCircularRefs.length > 0 && objectReplacementIterationCount < maxObjectReplacementIterations) {
                this.debug(`Object replacement iteration ${objectReplacementIterationCount + 1}: replacing remaining circular references.`);
                this.replaceAllRefsWithObject(schemas, remainingCircularRefs);
                // biome-ignore lint/performance/noAwaitInLoops: Awaiting here is necessary to ensure we get all of the remaining circular references
                remainingCircularRefs = await this.getCircularRefs(spec);
                this.debug(`After iteration ${objectReplacementIterationCount + 1}, remaining circular references: ${remainingCircularRefs.length}`);
    ...
    async run() {
        ...
        const parsedSpec = JSON.parse(preparedSpec);
