...
export default class OpenAPIUploadCommand extends BaseCommand {
    id = 'openapi upload';
    ...
    static flags = {
        ...
        'confirm-overwrite': Flags.boolean({
            description: 'If set, file overwrites will be made without a confirmation prompt. This flag can be a useful in automated environments where prompts cannot be responded to.',
        }),
        'dry-run': Flags.boolean({
            summary: 'Outputs what would be done, but does not actually upload anything.',
            description: 'If set, the command will output what would be done, but does not actually upload anything. This can be useful to verify that the correct file will be uploaded and the correct branch will be used.',
            deprecateAliases: true,
            aliases: ['dryRun'],
        }),
        'legacy-id': Flags.string({
            summary: 'The legacy ID for your API definition.',
            description: 'This is only used for legacy `rdme` CLI workflows and only applies if your project, and this API definition, predate ReadMe Refactored. This flag is considered deprecated and we recommend using `--slug` instead.',
            hidden: true,
            exclusive: ['slug'],
        }),
        slug: Flags.string({
        ...
        useSpecVersion: Flags.boolean({
            ...
            description: 'If included, use the version specified in the `info.version` field in your OpenAPI definition for your ReadMe project version. This flag is mutually exclusive with `--branch`.',
            exclusive: ['branch'],
        }),
    ...
    static examples = [
        ...
        {
            description: 'You can also pass in a file in a subdirectory (we recommend always running the CLI from the root of your repository):',
        ...
        {
            description: 'If you specify your ReadMe project version in the `info.version` field in your OpenAPI definition, you can use that:',
     ...
     */
    async pollAPIUntilUploadIsComplete(slug, headers) {
        let count = 0;
        let status = 'pending';
    ...
    async run() {
        ...
        const existingAPIDefinitions = (await this.readmeAPIFetch(`/branches/${branch}/apis`, { headers }).then(res => this.handleAPIRes(res)))?.data ||
            [];
        ...
        if (this.flags.slug) {
            ...
            if (slugExtension) {
                if (!['.json', ...yamlExtensions].includes(slugExtension)) {
                    throw new Error('Please provide a valid file extension that matches the extension on the file you provided. Must be `.json`, `.yaml`, or `.yml`.');
                }
                ...
                if (fileExtension !== slugExtension && !(isFileYaml && yamlExtensions.includes(slugExtension))) {
                    ...
                    if (fileExtension) {
                        this.warn(`The file extension in your provided slug (${slugExtension}) does not match the file extension of the file you're uploading (${fileExtension}). Your API definition will be converted to ${isFileYaml ? 'JSON' : 'YAML'} prior to upload.`);
                    }
                }
            ...
            if (objectIdRegex.test(this.flags.slug)) {
                ...
                if (matchedLegacyAPIDefinition) {
                    if (isCI()) {
                        throw new Error(`The slug you provided matches a legacy API definition ID, which have been deprecated in ReadMe. To fix this, update your \`--slug\` flag to  \`${matchedLegacyAPIDefinition.filename}\`.`);
                    }
                    ...
                    this.warn(`The slug you provided matches a legacy API definition ID and these IDs are now deprecated in ReadMe. To ensure maximum compatibility going forward, we recommend updating your \`--slug\` flag to  \`${matchedLegacyAPIDefinition.filename}\`.`);
                    filename = matchedLegacyAPIDefinition.filename;
                }
                else {
                    this.warn('The slug you provided looks like a legacy API definition ID, and these IDs are now deprecated in ReadMe. More info here: https://github.com/step-security/rdme/blob/v10/documentation/migration-guide.md#v10-openapi-upload-command-updates');
                }
            }
        }
        ...
        if (!filename) {
            ...
            this.warn(`No filename could be inferred from the provided URL, so the slug will default to ${filename}. To set a custom slug, use the \`--slug\` flag.`);
        }
        ...
        // warn them that the slug may not be what they expect
            ...
            this.warn(`The slug of your API Definition will be set to ${filename} in ReadMe. This slug is not visible to your end users. To set this slug to something else, use the \`--slug\` flag.`);
        }
        ...
        if (this.flags['legacy-id']) {
            if (!matchingAPIDefinition) {
                let existingDefinitionSuggestion = 'No existing definitions were found. Did you mean to use the `--slug` flag instead?';
                ...
                throw new Error([`No API definition found with legacy ID ${this.flags['legacy-id']}.`, existingDefinitionSuggestion].join(' '));
            }
            filename = matchingAPIDefinition.filename;
            this.warn(`The \`--legacy-id\` flag will be removed in a future version. We recommend passing \`--slug ${filename}\` for maximum compatibility and readability.`);
        ...
        const spinner = ora({ ...oraOptions() }).start(`${method === 'POST' ? 'Creating' : 'Updating'} your API definition to ReadMe...`);
        const response = (await this.readmeAPIFetch(`/branches/${branch}/apis${method === 'POST' ? '' : `/${filename}`}`, options)
        ...
        if (response?.data?.upload?.status && response?.data?.uri) {
            ...
            if (status === 'done') {
                ...
                this.log(`🚀 Your API definition (${filename}) was successfully ${method === 'POST' ? 'created' : 'updated'} in ReadMe!`);
                return { uri: response.data.uri, status };
            }
        }
        spinner.fail();
        throw new Error('Your API definition upload failed with an unexpected error. Please get in touch with us at support@readme.io.');
    }
}
