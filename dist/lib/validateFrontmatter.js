...
export async function validateFrontmatter(pages, outputDir) {
    ...
    if (!filesWithIssues.length) {
        ...
        if (filesWithFixableIssues.length) {
            if (confirmAutofixes) {
                ...
                throw new Error(`${filesWithIssues.length} file(s) have issues that should be fixed before uploading to ReadMe. Please run \`${this.config.bin} ${this.id} ${pathInput} --dry-run\` in a non-CI environment to fix them.`);
            }
            ...
            pagesToReturn.forEach((file, index) => {
                ...
                if (currentValidationResult.fixableErrorCount) {
                    this.log(`- ${file.filePath} (${currentValidationResult.fixableErrorCount} issue${currentValidationResult.fixableErrorCount > 1 ? 's' : ''})`);
                }
            });
            this.log();
            this.log(`Please review the changes. Once everything looks good, re-run \`${chalk.blue(`${this.config.bin} ${this.id}`)}\` to upload these files to ReadMe.`);
        }
        // also inform the user if there are files with issues that can't be fixed
        if (filesWithUnfixableIssues.length) {
            if (status !== 'autofixed') {
                this.warn(`${filesWithUnfixableIssues.length} file(s) have issues that cannot be fixed automatically. The upload will proceed but we recommend addressing these issues. Please get in touch with us at support@readme.io if you need a hand.`);
            }
            else {
                status = 'autofixed-with-issues';
                this.warn(`${filesWithUnfixableIssues.length} file(s) have issues that cannot be fixed automatically. Autofixable issues have been corrected but we also recommend addressing these issues as well. Please get in touch with us at support@readme.io if you need a hand.`);
            }
        }
    }
