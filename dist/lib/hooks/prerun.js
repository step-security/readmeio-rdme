...
const hook = async function run(options) {
            ...
            options.Command.flags.key = Flags.string({
                ...
                parse: async (input) => {
                    ...
                    if (input && configstore.get('apiKey') === input) {
                        info(`🔑 ${chalk.green(email)} is currently logged in, using the stored API key for this project: ${chalk.blue(project)}`, { includeEmojiPrefix: false });
                    }
                    return input;
                },
