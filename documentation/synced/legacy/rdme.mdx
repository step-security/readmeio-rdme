---
title: Syncing Docs via CLI / GitHub (Legacy)
content:
...
1. Download `rdme` to your local machine (see [our setup instructions](https://github.com/step-security/rdme#setup))
...
pipelines:
  default:
    - step:
        script:
          - npx rdme@v9 openapi validate [url-or-local-path-to-file] --key=$README_API_KEY --id=API_DEFINITION_ID
```
...
jobs:
  sync-via-rdme:
    ...
    steps:
      - run:
          command: npx rdme@v9 openapi validate [url-or-local-path-to-file] --key=$README_API_KEY --id=API_DEFINITION_ID
```
...
script: npx rdme@v9 openapi validate [url-or-local-path-to-file] --key=$README_API_KEY --id=API_DEFINITION_ID
```
{/* prettier-ignore-end */}
